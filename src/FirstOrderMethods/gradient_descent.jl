...
mutable struct GradientDescentSolver{T, V} <: SolverCore.AbstractOptimizationSolver 
...
function GradientDescentSolver(nlp::AbstractNLPModel{T, V}; alpha = 1e-3) where {T, V}
    x = copy(nlp.meta.x0) # iterate
    g = similar(nlp.meta.x0) # gradient

    return GradientDescentSolver{T, V}(x, g, T(alpha))
end

function SolverCore.solve!(solver::GradientDescentSolver, 
